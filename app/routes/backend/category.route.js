...
const serviceCategory = require(__path_services_backend + `${mainName}.service`);
...
router.get('(/status/:status)?', async (req, res, next) => {
    ...
    if (keyword !== '') objWhere.name = new RegExp(keyword, 'i');
		pagination.totalItems = await serviceCategory.countItem(objWhere);

			let data = await serviceCategory.listItems(objWhere, 
...
router.post('/save/(:id)?',
	body('name')
	  .isLength({min: 5, max: 100})
		...
		.custom(async (val, {req}) => {
			...
			let data		= await serviceCategory.checkDuplicatedName(val)
	...
	body('slug')
		...
		.custom(async (val, {req}) => {
			let paramId = await(req.params.id != undefined) ? req.params.id : 0
			let data		= await serviceCategory.checkDuplicatedSlug(val)
			let length = data.length
			data.forEach((value, index) => {
				if (value.id == paramId) 
					length = length - 1;
	...
	async function (req, res) { // Finds the validation errors in this request and wraps them in an object with handy functions
			...
			let itemData = [{}]
			if(req.params.id != undefined){
				itemData = await serviceCategory.getItemByID(req.params.id)
			}
			...
			if(!errors.isEmpty()) {
				...
				if (req.params.id !== undefined) {
					let data = await serviceCategory.editItem(req.params.id, item)
					req.flash('success', notify.EDIT_SUCCESS);
					res.redirect(linkIndex);
				} else {
					let data = await serviceCategory.saveItems(item);
...
router.post('/delete/(:status)?', async (req, res, next) => {
	try {
		if (req.params.status === 'multi') {
			...
			let data = await serviceCategory.deleteItemsMulti(arrId);
			res.send({success: true})
	} else {
			let id = req.body.id
			let data = await serviceCategory.deleteItem(id);
			res.send({success: true})
	}
	} catch (error) {
		console.log(error)
...
router.post('/change-status/(:status)?', async (req, res, next) => {
        ...
        console.log(status)
        let data = await serviceCategory.changeStatusItemsMulti(arrId, status);
        res.send({success: true})
    } else {
        let {status, id} = req.body
        status = (status == 'active') ? 'inactive' : 'active'
        let changeStatus = await serviceCategory.changeStatus(id, status)
        res.send({success: true})
    }
		} catch (error) {
			console.log(error)
...
router.post('/change-ordering', 
	...
	async (req, res, next) => {
		try {
			...
			let changeStatus = await serviceCategory.changeOrdering(id, ordering)
			res.send({success: true})
		} catch (error) {
			console.log(error)
		}
